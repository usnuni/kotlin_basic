...
fun  irisClassification() {
    ...
    model.use {
        it.compile(SGD(), Losses.SOFT_MAX_CROSS_ENTROPY_WITH_LOGITS, Metrics.ACCURACY)
        it.fit(train, EPOCHS, TRAINING_BATCH_SIZE)

        val accuracy = model.evaluate(test, TEST_BATCH_SIZE).metrics[Metrics.ACCURACY]
...
/*
function 파트 보면 이 함수를 이용해서 리턴값을 받는데 여기 들어가는 객체는 array여야한다
객체를 넣으면 x, y 를 이용한 데이터셋을 만들어주게 된다.
 */
